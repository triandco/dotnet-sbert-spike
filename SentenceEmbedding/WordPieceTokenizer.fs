...
let tokenize (options: WordPieceTokeniserOptions) text = 
    ...
    let toToken (token: string) : string seq =
      if String.length token > options.MaxInputCharPerWord then [options.UNKToken] else
      
      let rec getToken (startIndex: int) (chars: char seq) : Option<string seq> = 
        let endIndex = Seq.length chars
        if startIndex >= endIndex then Some [] else
        
          let rec getSubToken (begining: int) (ending: int) (charss: char seq) : Option<string * int> =
            ...
            else getSubToken begining (ending - 1) charss

          match getSubToken startIndex endIndex chars with 
          | Some (substr, ending) ->
             let t = getToken ending chars |> Option.map (Seq.append [| substr |])
             t
          | None -> None

      getToken 0 token |> Option.defaultValue [ options.UNKToken ]
